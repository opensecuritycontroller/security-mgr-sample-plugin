...
create table if not exists SECURITY_GROUP ( id bigint not null, name varchar(255), device_fk bigint not null, primary key (id) );

create table if not exists SECURITY_GROUP_INTERFACE ( id bigint not null, name varchar(255), tag varchar(255), security_group_fk bigint, device_fk bigint not null, primary key (id) );
...
alter table SECURITY_GROUP_INTERFACE add constraint if not exists FK_SGI_SG foreign key (security_group_fk) references SECURITY_GROUP;
